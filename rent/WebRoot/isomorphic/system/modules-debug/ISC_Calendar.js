...
// Refreshes the event's canvas in the current view.  Has no effect in the
// +link{calendar.monthView, monthView}.
//
...
// Has no effect in the +link{calendar.monthView, monthView}.
//
// @param event       (CalendarEvent) The event object to refresh in the current view
...
_getNewEventWindow : function (event, eventIndex, view) {
    ...
    if (canvas) {
        ...
        var canvasClass = this.eventCanvasConstructor;
        canvas = this.createAutoChild("eventWindow", props, canvasClass);
    }
                    ...
                    var fieldList = [
                        ...
                        {name: "details", title: cal.detailsButtonTitle, type: "button", startRow: false,
                            click : function (form, item) {
                                ...
                                ;
                                if (isNew) {
                                    event[cal.startDateField] = cal.eventDialog.currentStart;
                                    event[cal.endDateField] = cal.eventDialog.currentEnd;
...
gridProps: {
    ...
    eventsOverlap : function (rangeObject, event, sameLaneOnly) {
        ...
        if (!a[startField] || !b[startField]) return false;
    ...
    sizeEventWindow : function (eventWin, forceRedraw) {
        ...
        if (!event || Array.isLoading(event)) return;
    ...
    refreshVisibleEvents : function (events) {
        ...
        var clearThese = this.poolEventWindows ? this._drawnEvents.duplicate() : [],
            addThese = []
        ;
        ...
        for (var i = 0; i < eventsLen; i++) {
            ...
            if (alreadyVisible) {
                ...
                //if (this.isGrouped) {
                    ...
                    this.sizeEventWindow(canvas, true);
                //}
                continue;
            }
        ...
        if (this.poolEventWindows && clearThese.length > 0) {
            // we want to clear eventCanvases that are no longer in the viewport
    ...
    removeEvent : function (event) {
        var canvas = this.getCurrentEventCanvas(event);
        if (canvas) {
            this.clearEventCanvas(canvas, !this.poolEventWindows);
            return true;
        } else {
            return false;
        }
    ...
    poolEventCanvas : function (canvas) {
        ...
        if (this.body) {
            if (canvas.event) {
                this.getCalendar().setEventCanvasID(this, canvas.event, null);
                canvas.event = null;
            }
    ...
    clearEventCanvas : function (eventCanvas, destroy) {
        ...
        if (eventCanvas) {
            ...
            while (--len >= 0) {
                ...
                if (this.poolEventWindows && !destroy) {
    ...
    clearEvents : function (start, destroy) {
        ...
        if (destroy == null) destroy = !this.poolEventWindows;
...
isc.DaySchedule.addProperties({
    ...
    refreshEvents : function () {
        ...
        if (arr.length > 0) {
            ...
            this.clearEvents(0, !this.poolEventWindows);
        }
        ...
        for (var i=0; i< allEvents.getLength(); i++) {
            ...
            if (!isc.isA.String(event)) {
                if (!event[cal.startDateField]) {
                    //this.logWarn("NO startDate: \n\t" + isc.echoFull(event));
                    //continue;
                }
...
isc.EventWindow.addProperties({
    ...
    setEvent : function (event) {
        this.event = event;
        var styleName = event.eventWindowStyle || this.calendar.eventWindowStyle;
    ...
    */
    ...
    _getEventBreadth : function (eventWin) {
        ...
        var eStartCol = Math.max(0, this.getEventStartCol(event) + 1),
            tooLong = false
        ;
        // work with body.fields, rather than this.fields, because there may be multiple frozen fields
        // (and not just the one that was previously being catered for)
        if (eStartCol + grossCols > this.body.fields.length) {
            grossCols--;
            tooLong = true;
        }
        ...
        if (cal.sizeEventsToGrid == false) {
            ...
            if (!tooLong) {
                ...
                // if the event was off the right-hand side of the range, pin to the end of the timeline
                var remainder = Math.floor(minsLen % minsInACol);
                breadth += remainder;
            }
        }
    ...
    addEvent : function (event, eventIndex) {
        ...
        var canvasFound = (canvas != null);

        if (canvasFound) {
            this._eventCanvasPool.remove(canvas);
        } else if (this.poolEventWindows) {
            // no canvas currently showing this event - get one from the pool
            canvas = this.getPooledEventCanvas(event);
            if (canvas) {
                reclaimed = true;
                //canvas.VSnapOrigin = 0;
            }
        ...
        // add the canvas to the drawnCanvasList
        if (this._drawnCanvasList && !this._drawnCanvasList.contains(canvas))
            this._drawnCanvasList.add(canvas);
        if (this._drawnEvents && !this._drawnEvents.contains(event))
            this._drawnEvents.add(event);
