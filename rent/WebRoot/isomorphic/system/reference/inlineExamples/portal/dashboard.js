// Sub-class ListGrid to save edited grid state
isc.ClassFactory.defineClass("PortalGrid", "ListGrid").addMethods({
    updateEditNode : function (editContext, editNode) {
        ...
        if (!sort) removeProperties.add("initialSort");
        else setProperties.initialSort = sort;

        // Save filter criteria
        var criteria = this.getFilterEditorCriteria();
        if (!criteria) removeProperties.add("initialCriteria");
        else setProperties.initialCriteria = criteria;

        editContext.setNodeProperties(editNode, setProperties, true);
        if (removeProperties.length > 0) {
            editContext.removeNodeProperties(editNode, removeProperties, true);
        }
    }
});
...
isc.ListGrid.create({
    ...
    saveDashboard : function () {
        var editNodes = editPane.serializeAllEditNodes();
